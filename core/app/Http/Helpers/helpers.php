...
function getLatestVersion()
{
    ...
    $url = 'https://license.ayondev.com/updates/version/' . systemDetails()['name'];
...
function showAmount($amount, $currency = null, $separate = true, $exceptZeros = false){
...
function cryptoQR($wallet)
{
    ...
    return "https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=$wallet&choe=UTF-8&chf=bg,s,FFFFFF00";
}
...
function getTemplates()
{
    $param['purchasecode'] = env("PURCHASECODE");
    $param['website'] = @$_SERVER['HTTP_HOST'] . @$_SERVER['REQUEST_URI'] . ' - ' . env("APP_URL");
    $url = 'https://license.ayondev.com/updates/templates/' . systemDetails()['name'];
    $result = curlPostContent($url, $param);
    if ($result) {
        return $result;
    } else {
